...
def build_subtitle_only_final_mux_command(
    source_video: Path,
    subtitles_srt: Path,
    output_mp4: Path,
    subtitle_language: str = "jpn",
...
def build_burned_subtitle_only_mux_command(
    source_video: Path,
    subtitle_overlays: Iterable[Mapping[str, object]],
    output_mp4: Path,
    duration_ms: Optional[int] = None,
    ...
    for overlay in overlays:
        command.extend(["-loop", "1", "-i", str(overlay["image"])])
    if overlays:
        filter_complex, video_map = _build_subtitle_overlay_filter(overlays, first_overlay_input_index=1)
        command.extend(["-filter_complex", filter_complex, "-map", video_map, "-map", "0:a:0?"])
    else:
        command.extend(
            [
                "-map",
...
def _build_subtitle_overlay_filter(
    overlays: List[Mapping[str, object]],
    first_overlay_input_index: int = 2,
) -> Tuple[str, str]:
    chains = []
    current_label = "0:v"
    for index, overlay in enumerate(overlays):
        input_index = index + first_overlay_input_index
...
def build_project_subtitle_only_final_mux_command(
    project_dir: Path,
    source_video: Optional[Path] = None,
    subtitles_srt: Optional[Path] = None,
    output_mp4: Optional[Path] = None,
    ...
    source = source_video or project / "input" / "source.mp4"
    subtitles = subtitles_srt or project / subtitle_artifacts(target_lang)["srt"]
    output = output_mp4 or project / subtitle_only_soft_output_artifact(target_lang)
    assert_inside_project(project, source)
    assert_inside_project(project, subtitles)
    assert_inside_project(project, output)
    return build_subtitle_only_final_mux_command(
        source_video=source,
        subtitles_srt=subtitles,
        output_mp4=output,
        subtitle_language=subtitle_metadata_language(target_lang),
