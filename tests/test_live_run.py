...
def test_run_live_pipeline_can_render_subtitles_without_tts_or_dubbed_audio(tmp_path):
    source_video = tmp_path / "source.mp4"
    source_video.write_bytes(b"fake-video")
    project_dir = tmp_path / "project"
    commands = []
    ...
    class FakeClient:
        ...
        def post_json_bytes(self, path, payload):
            raise AssertionError("subtitle-only mode must not call xAI TTS")
    ...
    assert result["validation"]["status"] == "complete"
    assert result["validation"]["final_output"] == "output/subtitled.ja.burned.mp4"
    ...
    assert commands[0][-1] == str(project_dir / "media" / "source_audio_for_stt.wav")
    assert commands[1][-1] == str(project_dir / "output" / "subtitled.ja.burned.mp4")
